...
def train_fn(data_loader , model , optimizer , device , scheduler):
    ...
    for bi , d in enumerate(tk0):
        ...
        #prints avg.loss in every iteration
...
def eval_fn(data_loader , model  , device):
    #we are not calculating loss for validation , we can implement if needed
    model.eval()
    fin_output_start = []
    fin_output_end = []
    fin_padding_lens = []
    ...
    for bi , d in enumerate(data_loader):
        ...
        #since we are not monitoring losses , we can remove targets
        #targets_start = d["targets_start"]
        #targets_end = d["targets_end"]
    ...
    for j in range(len(fin_tweet_tokens)):
        ...
        if(len(padding) > 0):
            ...
            mask_end = fin_output_start[j,:][:-padding_len] >= threshold
        else:
            mask_start = fin_output_start[j,:] >= threshold
            mask_end = fin_output_start[j,:] >= threshold
